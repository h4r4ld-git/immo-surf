function profile(){
  return `<div class="profiles-container" style="height: 350px;">
                            ...
                            <div class="card-block">
                                ...
                                <div class="row">
                                    <div class="col-sm-6" style="float:left; width:40%">
                                        ...
                                        <?php
                                        $result1 = $wpdb->get_results("SELECT * FROM affiches");
                                        $i = 0;
                                        foreach($result1 as $aff){
                                          if (($pmail == $aff->mail) || ($pmail == "admin@boscop")){
                                            $i = $i + 1;
                                          };
                                        }
                                        ?>
                                        <h6 class="text-muted f-w-600" style="float:left;"><?php echo $i?></h6>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="profiles-container">
      <div class="profiles-list scroll">
        ...
        foreach($result1 as $aff){
          if (($pmail == $aff->mail) || ($pmail == "admin@boscop")){
            echo "<input id='but" . $i . "' class='profile-button' type='button' name='" . $aff->address . "' value='" . $aff->address . "' onclick='aff(event)'>";
            $affs[$aff->address] = [$aff->title, $aff->description, $aff->mail, $aff->tel, $aff->prix, $aff->exp, $aff->address];
            $i = $i + 1;
          };
        }
        $js_array = json_encode($affs);
        echo "<script>var affs = ". $js_array . ";</script>\n";
        ?>
